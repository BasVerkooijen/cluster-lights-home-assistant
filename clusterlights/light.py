"""Support for Action christmas lights."""
import logging
...
SUPPORT_CHRISTMASLIGHTS_LED = SUPPORT_BRIGHTNESS | SUPPORT_EFFECT
...
def setup_platform(hass, config, add_entities, discovery_info=None):
	"""Set up the christmas lights."""
	lights = []
	for address, device_config in config[CONF_DEVICES].items():
		device = {}
		device["name"] = device_config[CONF_NAME]
		device["address"] = address
		light = ChristmasLights(device)
		if light.is_valid:
			lights.append(light)
...
class ChristmasLights(Light):
	"""Representation of Christmas lights."""
	...
	def __init__(self, device):
		...
		self.is_valid = True
		self._bulb = christmaslights(self._address)
		self._brightness = 0
		self._state = False
		...
		if self._bulb.connect() is False:
			self.is_valid = False
			_LOGGER.error("Failed to connect to christmas lights %s, %s", self._address, self._name)
			return
	...
	def supported_features(self):
		...
		return SUPPORT_CHRISTMASLIGHTS_LED
